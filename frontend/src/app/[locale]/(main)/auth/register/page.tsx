...
export default function Register() {
  ...
  const getTranslatedError = (error: string) =>
    error.toLowerCase().includes('already registered') ? t('emailAlreadyRegistered') : error
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      if (response.error) {
        setApiError(getTranslatedError(response.error))
        return
      }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (registerResponse.error) {
        ...
        setOtpError(getTranslatedError(registerResponse.error))
        return
      }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (response.error) {
        setOtpError(getTranslatedError(response.error))
        return
      }
